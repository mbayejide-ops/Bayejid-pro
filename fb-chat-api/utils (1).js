...
/**
 * Fallback mention resolver.
 * Used when native mention data (m.delta.data.prng) is missing/empty from Facebook's payload,
 * but the message body still contains "@Name" text.
 * Matches "@Name" substrings in the body against the thread's member list by name.
 * Returns a mentions object in the same shape as formatDeltaMessage: { userID: "@Name" }
 */
async function resolveMentionsFromBody(api, threadID, body) {
  const mentions = {};
  if (!body || !body.includes("@") || !threadID) return mentions;

  try {
    const threadInfo = await api.getThreadInfo(threadID);
    const members = (threadInfo && threadInfo.userInfo) || [];
    if (!members.length) return mentions;

    // Sort candidates by name length descending so longer/more specific names
    // are matched before shorter substrings of them (e.g. "Md Rakib" before "Rakib").
    const candidates = members
      .filter(u => u.name)
      .sort((a, b) => b.name.length - a.name.length);

    // Find every "@Something" occurrence in the body
    const tagPattern = /@([^\s@][^@]{0,60}?)(?=(\s@|$|[\n,.!?]))/g;
    let match;
    while ((match = tagPattern.exec(body)) !== null) {
      const raw = match[1].trim();
      if (!raw) continue;

      // Try to find the best matching member name that the tagged text starts with
      // (handles cases where the raw match includes trailing words not part of the name)
      let bestMatch = null;
      ...
      if (bestMatch && !mentions[bestMatch.id]) {
        mentions[bestMatch.id] = "@" + bestMatch.name;
      }
    }
  } catch (err) {
    // Silently fail — fallback should never crash message handling
  }
