		...
		(function resolveAttachmentUrl(i) {
			if (i == (v.delta.attachments || []).length) {
				...
				return !ctx.globalOptions.selfListen &&
					...
					undefined :
					(async function () {
						// Fallback: if native mention data was empty but the body still
						// contains "@Name" text, try to resolve mentions by matching
						// thread member names. Never blocks/crashes on failure.
						if (
							fmtMsg.mentions &&
							Object.keys(fmtMsg.mentions).length === 0 &&
							fmtMsg.body &&
							fmtMsg.body.includes("@")
						) {
							try {
								const fallbackMentions = await utils.resolveMentionsFromBody(
									api,
									fmtMsg.threadID,
									fmtMsg.body
								);
								if (fallbackMentions && Object.keys(fallbackMentions).length > 0) {
									fmtMsg.mentions = fallbackMentions;
								}
							} catch (err) {
								// Ignore — fall through with original (empty) mentions
							}
						}
